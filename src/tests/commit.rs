...
use super::{make_test_repo, run_git, git_stdout, git_commit_tree_with_env, test_commit_identity};
...
fn commit_on_branch_first_then_second() {
    let repo = make_test_repo("commit_branch_twice");
    let id = test_commit_identity();

    fs::write(repo.worktree.join("track.txt"), "v1\n").unwrap();
    ...
    let head_git = git_stdout(&repo.worktree, &["rev-parse", "HEAD"])
    ...
    let head2 = git_stdout(&repo.worktree, &["rev-parse", "HEAD"])
        .lines().next().unwrap().trim().to_string();
...
fn commit_on_detached_head_updates_oid_and_parent() {
    let repo = make_test_repo("commit_detached");
    let id = test_commit_identity();

    // 准备种子 commit，再手动制造 detached HEAD
    fs::write(repo.worktree.join("only.txt"), "a\n").unwrap();
    run_git(&repo.worktree, &["init"]);
    run_git(&repo.worktree, &["add", "only.txt"]);
    let tree_hex = git_stdout(&repo.worktree, &["write-tree"])
        .lines().next().unwrap().trim().to_string();
    let c0 = git_commit_tree_with_env(&repo.worktree, &tree_hex, &[], "seed");
    fs::write(repo.worktree.join(".git/HEAD"), format!("{c0}\n")).unwrap(); // detached HEAD
