...
use crate::status;
use super::{make_case_dir, run_git, test_git_dir};
...
fn test_status_basic() {
    ...
    let result = status::status(worktree, git_dir).expect("status failed");
...
fn test_status_functionality() {
    ...
    // 场景 1：干净状态
    let result = crate::status::status(worktree, git_dir).expect("status");
    ...
    run_git(worktree, &["add", "file1.txt"]);                             // stage file1
